...
void fatal(const char *s, ...)    // failure exit
{
    ...
    if(errors <= 2) // print up to one extra recursive error
    {
        ...
        if(errors <= 1) // avoid recursion
        {
            ...
            SDL_Quit();
            SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_ERROR, "somegame fatal error", msg, NULL);
        }
    }
...
void setupscreen()
{
    ...
    screen = SDL_CreateWindow("somegame", winx, winy, winw, winh, SDL_WINDOW_OPENGL | SDL_WINDOW_SHOWN | SDL_WINDOW_INPUT_FOCUS | SDL_WINDOW_MOUSE_FOCUS | flags);
...
void stackdumper(unsigned int type, EXCEPTION_POINTERS *ep)
{
    ...
    char out[512];
    formatstring(out, "somegame Win32 Exception: 0x%x [0x%x]\n\n", er->ExceptionCode, er->ExceptionCode==EXCEPTION_ACCESS_VIOLATION ? er->ExceptionInformation[1] : -1);
